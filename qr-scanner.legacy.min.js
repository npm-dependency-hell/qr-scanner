...
c.brands.some(({brand:d,version:f})=>/Chromium/i.test(d)&&113>parseInt(f))&&/mac ?OS/i.test(c.platform)&&(yield c.getHighEntropyValues(["architecture","platformVersion"]).then(({architecture:d,platformVersion:f})=>/arm/i.test(d||"arm")&&13<=parseInt(f||"13")).catch(()=>!0))?a():new BarcodeDetector({formats:["qr_code"]})})}_onPlay(){this._scanRegion=this._calculateScanRegion(this.$video);this._updateOverlay();this.$overlay&&(this.$overlay.style.display="");this._scanFrame()}_onLoadedMetaData(){this._scanRegion=
this._calculateScanRegion(this.$video);this._updateOverlay()}_onVisibilityChange(){document.hidden?this.pause():this._active&&this.start()}_calculateScanRegion(a){let c=Math.round(2/3*Math.min(a.videoWidth,a.videoHeight));return{x:Math.round((a.videoWidth-c)/2),y:Math.round((a.videoHeight-c)/2),width:c,height:c,downScaledWidth:this._legacyCanvasSize,downScaledHeight:this._legacyCanvasSize}}_updateOverlay(){requestAnimationFrame(()=>{if(this.$overlay){var a=this.$video,c=a.videoWidth,d=a.videoHeight,
f=a.offsetWidth,g=a.offsetHeight,h=a.offsetLeft,r=a.offsetTop,p=window.getComputedStyle(a),q=p.objectFit,l=c/d,t=f/g;switch(q){case "none":var m=c;var k=d;break;case "fill":m=f;k=g;break;default:("cover"===q?l>t:l<t)?(k=g,m=k*l):(m=f,k=m/l),"scale-down"===q&&(m=Math.min(m,c),k=Math.min(k,d))}var [n,z]=p.objectPosition.split(" ").map((v,w)=>{const y=parseFloat(v);return v.endsWith("%")?(w?g-k:f-m)*y/100:y});p=this._scanRegion.width||c;t=this._scanRegion.height||d;q=this._scanRegion.x||0;var x=this._scanRegion.y||
0;l=this.$overlay.style;l.width=`${p/c*m}px`;l.height=`${t/d*k}px`;l.top=`${r+z+x/d*k}px`;d=/scaleX\(-1\)/.test(a.style.transform);l.left=`${h+(d?f-n-m:n)+(d?c-q-p:q)/c*m}px`;l.transform=a.style.transform}})}static _convertPoints(a,c){if(!c)return a;let d=c.x||0,f=c.y||0,g=c.width&&c.downScaledWidth?c.width/c.downScaledWidth:1;c=c.height&&c.downScaledHeight?c.height/c.downScaledHeight:1;for(let h of a)h.x=h.x*g+d,h.y=h.y*c+f;return a}_scanFrame(){!this._active||this.$video.paused||this.$video.ended||
