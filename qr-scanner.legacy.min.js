...
c._preferredCamera&&(c._preferredCamera=a,yield c._restartVideoStream())})}static scanImage(a,c){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let d,f,g,h,r=!1,p=!1;c="object"===typeof c?c:{};d=c.scanRegion||d;f=c.qrEngine||f;g=c.canvas||g;h=c.fullCanvas||h;r=c.disallowCanvasResizing||!1;p=c.alsoTryWithoutScanRegion||!1;let q=!!f;try{let l,t;[f,l]=yield Promise.all([f||b.createQrEngine(),b._loadImage(a)]);[g,t,h]=b._drawToCanvas(l,d,g,h,r);let m;if(f instanceof Worker){let k=f;q||
b._postWorkerMessageSync(k,"inversionMode","both");m=yield new Promise((n,z)=>{let x,v,w,y=-1;v=u=>{u.data.id===y&&(k.removeEventListener("message",v),k.removeEventListener("error",w),clearTimeout(x),null!==u.data.data?n({data:u.data.data,cornerPoints:b._convertPoints(u.data.cornerPoints,d),image:h}):z(b.NO_QR_CODE_FOUND))};w=u=>{k.removeEventListener("message",v);k.removeEventListener("error",w);clearTimeout(x);z("Scanner error: "+(u?u.message||u:"Unknown Error"))};k.addEventListener("message",v);
k.addEventListener("error",w);x=setTimeout(()=>w("timeout"),1E4);let A=t.getImageData(0,0,g.width,g.height);y=b._postWorkerMessageSync(k,"decode",A,[A.data.buffer])})}else m=yield Promise.race([new Promise((k,n)=>window.setTimeout(()=>n("Scanner error: timeout"),1E4)),(()=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){try{var k=t.getImageData(0,0,g.width,g.height);let [n]=yield f.detect(k);if(!n)throw b.NO_QR_CODE_FOUND;return{data:n.rawValue,cornerPoints:b._convertPoints(n.cornerPoints,
d),image:h}}catch(n){k=n.message||n;if(/not implemented|service unavailable/.test(k))return b._disableBarcodeDetector=!0,b.scanImage(a,{scanRegion:d,canvas:g,disallowCanvasResizing:r,alsoTryWithoutScanRegion:p});throw`Scanner error: ${k}`;}}))()]);return m}catch(l){if(!d||!p)throw l;return yield b.scanImage(a,{qrEngine:f,canvas:g,disallowCanvasResizing:r})}finally{q||b._postWorkerMessage(f,"close")}})}setGrayscaleWeights(a,c,d,f=!0){b._postWorkerMessage(this._qrEnginePromise,"grayscaleWeights",{red:a,
green:c,blue:d,useIntegerApproximation:f})}setInversionMode(a){b._postWorkerMessage(this._qrEnginePromise,"inversionMode",a)}static createQrEngine(){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let a=()=>Promise.resolve().then(function(){return e}).then(d=>d.createWorker());if(!(!b._disableBarcodeDetector&&"BarcodeDetector"in window&&BarcodeDetector.getSupportedFormats&&(yield BarcodeDetector.getSupportedFormats()).includes("qr_code")))return a();let c=navigator.userAgentData;return c&&
c.brands.some(({brand:d})=>/Chromium/i.test(d))&&/mac ?OS/i.test(c.platform)&&(yield c.getHighEntropyValues(["architecture","platformVersion"]).then(({architecture:d,platformVersion:f})=>/arm/i.test(d||"arm")&&13<=parseInt(f||"13")).catch(()=>!0))?a():new BarcodeDetector({formats:["qr_code"]})})}_onPlay(){this._scanRegion=this._calculateScanRegion(this.$video);this._updateOverlay();this.$overlay&&(this.$overlay.style.display="");this._scanFrame()}_onLoadedMetaData(){this._scanRegion=this._calculateScanRegion(this.$video);
this._updateOverlay()}_onVisibilityChange(){document.hidden?this.pause():this._active&&this.start()}_calculateScanRegion(a){let c=Math.round(2/3*Math.min(a.videoWidth,a.videoHeight));return{x:Math.round((a.videoWidth-c)/2),y:Math.round((a.videoHeight-c)/2),width:c,height:c,downScaledWidth:this._legacyCanvasSize,downScaledHeight:this._legacyCanvasSize}}_updateOverlay(){requestAnimationFrame(()=>{if(this.$overlay){var a=this.$video,c=a.videoWidth,d=a.videoHeight,f=a.offsetWidth,g=a.offsetHeight,h=a.offsetLeft,
r=a.offsetTop,p=window.getComputedStyle(a),q=p.objectFit,l=c/d,t=f/g;switch(q){case "none":var m=c;var k=d;break;case "fill":m=f;k=g;break;default:("cover"===q?l>t:l<t)?(k=g,m=k*l):(m=f,k=m/l),"scale-down"===q&&(m=Math.min(m,c),k=Math.min(k,d))}var [n,z]=p.objectPosition.split(" ").map((v,w)=>{const y=parseFloat(v);return v.endsWith("%")?(w?g-k:f-m)*y/100:y});p=this._scanRegion.width||c;t=this._scanRegion.height||d;q=this._scanRegion.x||0;var x=this._scanRegion.y||0;l=this.$overlay.style;l.width=
`${p/c*m}px`;l.height=`${t/d*k}px`;l.top=`${r+z+x/d*k}px`;d=/scaleX\(-1\)/.test(a.style.transform);l.left=`${h+(d?f-n-m:n)+(d?c-q-p:q)/c*m}px`;l.transform=a.style.transform}})}static _convertPoints(a,c){if(!c)return a;let d=c.x||0,f=c.y||0,g=c.width&&c.downScaledWidth?c.width/c.downScaledWidth:1;c=c.height&&c.downScaledHeight?c.height/c.downScaledHeight:1;for(let h of a)h.x=h.x*g+d,h.y=h.y*c+f;return a}_scanFrame(){!this._active||this.$video.paused||this.$video.ended||("requestVideoFrameCallback"in
this.$video?this.$video.requestVideoFrameCallback.bind(this.$video):requestAnimationFrame)(()=>{const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(!(1>=a.$video.readyState)){var c=Date.now()-a._lastScanTimestamp,d=1E3/a._maxScansPerSecond;c<d&&(yield new Promise(g=>setTimeout(g,d-c)));a._lastScanTimestamp=Date.now();try{var f=yield b.scanImage(a.$video,{scanRegion:a._scanRegion,qrEngine:a._qrEnginePromise,canvas:a.$canvas,fullCanvas:a.$fullCanvas})}catch(g){if(!a._active)return;
a._onDecodeError(g)}!b._disableBarcodeDetector||(yield a._qrEnginePromise)instanceof Worker||(a._qrEnginePromise=b.createQrEngine());f?(a._onDecode&&a._onDecode(f),a.$codeOutlineHighlight&&(clearTimeout(a._codeOutlineHighlightRemovalTimeout),a._codeOutlineHighlightRemovalTimeout=void 0,a.$codeOutlineHighlight.setAttribute("viewBox",`${a._scanRegion.x||0} `+`${a._scanRegion.y||0} `+`${a._scanRegion.width||a.$video.videoWidth} `+`${a._scanRegion.height||a.$video.videoHeight}`),a.$codeOutlineHighlight.firstElementChild.setAttribute("points",
f.cornerPoints.map(({x:g,y:h})=>`${g},${h}`).join(" ")),a.$codeOutlineHighlight.style.display="")):a.$codeOutlineHighlight&&!a._codeOutlineHighlightRemovalTimeout&&(a._codeOutlineHighlightRemovalTimeout=setTimeout(()=>a.$codeOutlineHighlight.style.display="none",100))}a._scanFrame()})})}_onDecodeError(a){a!==b.NO_QR_CODE_FOUND&&console.log(a)}_getCameraStream(){const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(!navigator.mediaDevices)throw"Camera not found.";let c=/^(environment|user)$/.test(a._preferredCamera)?
"facingMode":"deviceId",d=[{width:{min:1024}},{width:{min:768}},{}],f=d.map(g=>Object.assign({},g,{[c]:{exact:a._preferredCamera}}));for(let g of[...f,...d])try{let h=yield navigator.mediaDevices.getUserMedia({video:g,audio:!1}),r=a._getFacingMode(h)||(g.facingMode?a._preferredCamera:"environment"===a._preferredCamera?"user":"environment");return{stream:h,facingMode:r}}catch(h){}throw"Camera not found.";})}_restartVideoStream(){const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c=
a._paused;(yield a.pause(!0))&&!c&&a._active&&(yield a.start())})}static _stopVideoStream(a){for(let c of a.getTracks())c.stop(),a.removeTrack(c)}_setVideoMirror(a){this.$video.style.transform="scaleX("+("user"===a?-1:1)+")"}_getFacingMode(a){return(a=a.getVideoTracks()[0])?/rear|back|environment/i.test(a.label)?"environment":/front|user|face/i.test(a.label)?"user":null:null}static _drawToCanvas(a,c,d,f,g=!1){d=d||document.createElement("canvas");f=f||document.createElement("canvas");var h=c&&c.x?
c.x:0;let r=c&&c.y?c.y:0,p=c&&c.width?c.width:a.videoWidth||a.width,q=c&&c.height?c.height:a.videoHeight||a.height;var l=a.videoWidth||a.width;let t=a.videoHeight||a.height;g||(g=c&&c.downScaledWidth?c.downScaledWidth:p,c=c&&c.downScaledHeight?c.downScaledHeight:q,d.width!==g&&(d.width=g),d.height!==c&&(d.height=c),f.width!==l&&(f.width=l),f.height!==t&&(f.height=t));l=d.getContext("2d",{alpha:!1});l.imageSmoothingEnabled=!1;l.drawImage(a,h,r,p,q,0,0,d.width,d.height);h=f.getContext("2d",{alpha:!1});
h.imageSmoothingEnabled=!1;h.drawImage(a,0,0,f.width,f.height);return[d,l,f,h]}static _loadImage(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(a instanceof Image)return yield b._awaitImageLoad(a),a;if(a instanceof HTMLVideoElement||a instanceof HTMLCanvasElement||a instanceof SVGImageElement||"OffscreenCanvas"in window&&a instanceof OffscreenCanvas||"ImageBitmap"in window&&a instanceof ImageBitmap)return a;if(a instanceof File||a instanceof Blob||a instanceof URL||"string"===
typeof a){let c=new Image;c.src=a instanceof File||a instanceof Blob?URL.createObjectURL(a):a.toString();try{return yield b._awaitImageLoad(c),c}finally{(a instanceof File||a instanceof Blob)&&URL.revokeObjectURL(c.src)}}else throw"Unsupported image type.";})}static _awaitImageLoad(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a.complete&&0!==a.naturalWidth||(yield new Promise((c,d)=>{let f=g=>{a.removeEventListener("load",f);a.removeEventListener("error",f);g instanceof ErrorEvent?
d("Image load error"):c()};a.addEventListener("load",f);a.addEventListener("error",f)}))})}static _postWorkerMessage(a,c,d,f){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){return b._postWorkerMessageSync(yield a,c,d,f)})}static _postWorkerMessageSync(a,c,d,f){if(!(a instanceof Worker))return-1;let g=b._workerMessageId++;a.postMessage({id:g,type:c,data:d},{transfer:f});return g}}b.DEFAULT_CANVAS_SIZE=400;b.NO_QR_CODE_FOUND="No QR code found";b._disableBarcodeDetector=!1;b._workerMessageId=
