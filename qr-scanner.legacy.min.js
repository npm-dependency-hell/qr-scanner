...
b._stopVideoStream(d))}})}constructor(a,c,d,f,g){this._legacyCanvasSize=b.DEFAULT_CANVAS_SIZE;this._preferredCamera="environment";this._maxScansPerSecond=25;this._lastScanTimestamp=-1;this._destroyed=this._flashOn=this._paused=this._active=!1;this.$video=a;this.$canvas=document.createElement("canvas");this.$fullCanvas=document.createElement("canvas");d&&"object"===typeof d?this._onDecode=c:(d||f||g?console.warn("You're using a deprecated version of the QrScanner constructor which will be removed in the future"):
console.warn("Note that the type of the scan result passed to onDecode will change in the future. To already switch to the new api today, you can pass returnDetailedScanResult: true."),this._legacyOnDecode=c);c="object"===typeof d?d:{};this._onDecodeError=c.onDecodeError||("function"===typeof d?d:this._onDecodeError);this._calculateScanRegion=c.calculateScanRegion||("function"===typeof f?f:this._calculateScanRegion);this._preferredCamera=c.preferredCamera||g||this._preferredCamera;this._legacyCanvasSize=
"number"===typeof d?d:"number"===typeof f?f:this._legacyCanvasSize;this._maxScansPerSecond=c.maxScansPerSecond||this._maxScansPerSecond;this._onPlay=this._onPlay.bind(this);this._onLoadedMetaData=this._onLoadedMetaData.bind(this);this._onVisibilityChange=this._onVisibilityChange.bind(this);this._updateOverlay=this._updateOverlay.bind(this);a.disablePictureInPicture=!0;a.playsInline=!0;a.muted=!0;let h=!1;a.hidden&&(a.hidden=!1,h=!0);document.body.contains(a)||(document.body.appendChild(a),h=!0);d=
a.parentElement;if(c.highlightScanRegion||c.highlightCodeOutline){f=!!c.overlay;this.$overlay=c.overlay||document.createElement("div");g=this.$overlay.style;g.position="absolute";g.display="none";g.pointerEvents="none";this.$overlay.classList.add("scan-region-highlight");if(!f&&c.highlightScanRegion){this.$overlay.innerHTML='<svg class="scan-region-highlight-svg" viewBox="0 0 238 238" preserveAspectRatio="none" style="position:absolute;width:100%;height:100%;left:0;top:0;fill:none;stroke:#e9b213;stroke-width:4;stroke-linecap:round;stroke-linejoin:round"><path d="M31 2H10a8 8 0 0 0-8 8v21M207 2h21a8 8 0 0 1 8 8v21m0 176v21a8 8 0 0 1-8 8h-21m-176 0H10a8 8 0 0 1-8-8v-21"/></svg>';
try{this.$overlay.firstElementChild.animate({transform:["scale(.98)","scale(1.01)"]},{duration:400,iterations:Infinity,direction:"alternate",easing:"ease-in-out"})}catch(p){}d.insertBefore(this.$overlay,this.$video.nextSibling)}c.highlightCodeOutline&&(this.$overlay.insertAdjacentHTML("beforeend",'<svg class="code-outline-highlight" preserveAspectRatio="none" style="display:none;width:100%;height:100%;fill:none;stroke:#e9b213;stroke-width:5;stroke-dasharray:25;stroke-linecap:round;stroke-linejoin:round"><polygon/></svg>'),
this.$codeOutlineHighlight=this.$overlay.lastElementChild)}this._scanRegion=this._calculateScanRegion(a);requestAnimationFrame(()=>{let p=window.getComputedStyle(a);"none"===p.display&&(a.style.setProperty("display","block","important"),h=!0);"visible"!==p.visibility&&(a.style.setProperty("visibility","visible","important"),h=!0);h&&(console.warn("QrScanner has overwritten the video hiding style to avoid Safari stopping the playback."),a.style.opacity="0",a.style.width="0",a.style.height="0",this.$overlay&&
...
c._preferredCamera&&(c._preferredCamera=a,yield c._restartVideoStream())})}static scanImage(a,c,d,f,g,h=!1,p=!1){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let n,r=!1;c&&("scanRegion"in c||"qrEngine"in c||"canvas"in c||"disallowCanvasResizing"in c||"alsoTryWithoutScanRegion"in c||"returnDetailedScanResult"in c)?(n=c.scanRegion,d=c.qrEngine,f=c.canvas,h=c.disallowCanvasResizing||!1,p=c.alsoTryWithoutScanRegion||!1,r=!0):c||d||f||h||p?console.warn("You're using a deprecated api for scanImage which will be removed in the future."):
console.warn("Note that the return type of scanImage will change in the future. To already switch to the new api today, you can pass returnDetailedScanResult: true.");let k=!!d;try{let t,l;[d,t]=yield Promise.all([d||b.createQrEngine(),b._loadImage(a)]);[f,l,g]=b._drawToCanvas(t,n,f,g,h);let q;if(d instanceof Worker){let m=d;k||b._postWorkerMessageSync(m,"inversionMode","both");q=yield new Promise((u,z)=>{let x,y,v,A=-1;y=w=>{w.data.id===A&&(m.removeEventListener("message",y),m.removeEventListener("error",
v),clearTimeout(x),null!==w.data.data?u({data:w.data.data,cornerPoints:b._convertPoints(w.data.cornerPoints,n),image:g}):z(b.NO_QR_CODE_FOUND))};v=w=>{m.removeEventListener("message",y);m.removeEventListener("error",v);clearTimeout(x);z("Scanner error: "+(w?w.message||w:"Unknown Error"))};m.addEventListener("message",y);m.addEventListener("error",v);x=setTimeout(()=>v("timeout"),1E4);let B=l.getImageData(0,0,f.width,f.height);A=b._postWorkerMessageSync(m,"decode",B,[B.data.buffer])})}else q=yield Promise.race([new Promise((m,
u)=>window.setTimeout(()=>u("Scanner error: timeout"),1E4)),(()=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){try{var m=l.getImageData(0,0,f.width,f.height);let [u]=yield d.detect(m);if(!u)throw b.NO_QR_CODE_FOUND;return{data:u.rawValue,cornerPoints:b._convertPoints(u.cornerPoints,n),image:g}}catch(u){m=u.message||u;if(/not implemented|service unavailable/.test(m))return b._disableBarcodeDetector=!0,b.scanImage(a,{scanRegion:n,canvas:f,disallowCanvasResizing:h,alsoTryWithoutScanRegion:p});
throw`Scanner error: ${m}`;}}))()]);return r?q:q.data}catch(t){if(!n||!p)throw t;let l=yield b.scanImage(a,{qrEngine:d,canvas:f,disallowCanvasResizing:h});return r?l:l.data}finally{k||b._postWorkerMessage(d,"close")}})}setGrayscaleWeights(a,c,d,f=!0){b._postWorkerMessage(this._qrEnginePromise,"grayscaleWeights",{red:a,green:c,blue:d,useIntegerApproximation:f})}setInversionMode(a){b._postWorkerMessage(this._qrEnginePromise,"inversionMode",a)}static createQrEngine(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a&&
console.warn("Specifying a worker path is not required and not supported anymore.");let c=()=>Promise.resolve().then(function(){return e}).then(f=>f.createWorker());if(!(!b._disableBarcodeDetector&&"BarcodeDetector"in window&&BarcodeDetector.getSupportedFormats&&(yield BarcodeDetector.getSupportedFormats()).includes("qr_code")))return c();let d=navigator.userAgentData;return d&&d.brands.some(({brand:f,version:g})=>/Chromium/i.test(f)&&113>parseInt(g))&&/mac ?OS/i.test(d.platform)&&(yield d.getHighEntropyValues(["architecture",
"platformVersion"]).then(({architecture:f,platformVersion:g})=>/arm/i.test(f||"arm")&&13<=parseInt(g||"13")).catch(()=>!0))?c():new BarcodeDetector({formats:["qr_code"]})})}_onPlay(){this._scanRegion=this._calculateScanRegion(this.$video);this._updateOverlay();this.$overlay&&(this.$overlay.style.display="");this._scanFrame()}_onLoadedMetaData(){this._scanRegion=this._calculateScanRegion(this.$video);this._updateOverlay()}_onVisibilityChange(){document.hidden?this.pause():this._active&&this.start()}_calculateScanRegion(a){let c=
Math.round(2/3*Math.min(a.videoWidth,a.videoHeight));return{x:Math.round((a.videoWidth-c)/2),y:Math.round((a.videoHeight-c)/2),width:c,height:c,downScaledWidth:this._legacyCanvasSize,downScaledHeight:this._legacyCanvasSize}}_updateOverlay(){requestAnimationFrame(()=>{if(this.$overlay){var a=this.$video,c=a.videoWidth,d=a.videoHeight,f=a.offsetWidth,g=a.offsetHeight,h=a.offsetLeft,p=a.offsetTop,n=window.getComputedStyle(a),r=n.objectFit,k=c/d,t=f/g;switch(r){case "none":var l=c;var q=d;break;case "fill":l=
f;q=g;break;default:("cover"===r?k>t:k<t)?(q=g,l=q*k):(l=f,q=l/k),"scale-down"===r&&(l=Math.min(l,c),q=Math.min(q,d))}var [m,u]=n.objectPosition.split(" ").map((x,y)=>{const v=parseFloat(x);return x.endsWith("%")?(y?g-q:f-l)*v/100:v});n=this._scanRegion.width||c;t=this._scanRegion.height||d;r=this._scanRegion.x||0;var z=this._scanRegion.y||0;k=this.$overlay.style;k.width=`${n/c*l}px`;k.height=`${t/d*q}px`;k.top=`${p+u+z/d*q}px`;d=/scaleX\(-1\)/.test(a.style.transform);k.left=`${h+(d?f-m-l:m)+(d?c-
r-n:r)/c*l}px`;k.transform=a.style.transform}})}static _convertPoints(a,c){if(!c)return a;let d=c.x||0,f=c.y||0,g=c.width&&c.downScaledWidth?c.width/c.downScaledWidth:1;c=c.height&&c.downScaledHeight?c.height/c.downScaledHeight:1;for(let h of a)h.x=h.x*g+d,h.y=h.y*c+f;return a}_scanFrame(){!this._active||this.$video.paused||this.$video.ended||("requestVideoFrameCallback"in this.$video?this.$video.requestVideoFrameCallback.bind(this.$video):requestAnimationFrame)(()=>{const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(!(1>=
...
a._legacyOnDecode(f.data),a.$codeOutlineHighlight&&(clearTimeout(a._codeOutlineHighlightRemovalTimeout),a._codeOutlineHighlightRemovalTimeout=void 0,a.$codeOutlineHighlight.setAttribute("viewBox",`${a._scanRegion.x||0} `+`${a._scanRegion.y||0} `+`${a._scanRegion.width||a.$video.videoWidth} `+`${a._scanRegion.height||a.$video.videoHeight}`),a.$codeOutlineHighlight.firstElementChild.setAttribute("points",f.cornerPoints.map(({x:g,y:h})=>`${g},${h}`).join(" ")),a.$codeOutlineHighlight.style.display="")):
a.$codeOutlineHighlight&&!a._codeOutlineHighlightRemovalTimeout&&(a._codeOutlineHighlightRemovalTimeout=setTimeout(()=>a.$codeOutlineHighlight.style.display="none",100))}a._scanFrame()})})}_onDecodeError(a){a!==b.NO_QR_CODE_FOUND&&console.log(a)}_getCameraStream(){const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(!navigator.mediaDevices)throw"Camera not found.";let c=/^(environment|user)$/.test(a._preferredCamera)?"facingMode":"deviceId",d=[{width:{min:1024}},{width:{min:768}},
{}],f=d.map(g=>Object.assign({},g,{[c]:{exact:a._preferredCamera}}));for(let g of[...f,...d])try{let h=yield navigator.mediaDevices.getUserMedia({video:g,audio:!1}),p=a._getFacingMode(h)||(g.facingMode?a._preferredCamera:"environment"===a._preferredCamera?"user":"environment");return{stream:h,facingMode:p}}catch(h){}throw"Camera not found.";})}_restartVideoStream(){const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let c=a._paused;(yield a.pause(!0))&&!c&&a._active&&(yield a.start())})}static _stopVideoStream(a){for(let c of a.getTracks())c.stop(),
a.removeTrack(c)}_setVideoMirror(a){this.$video.style.transform="scaleX("+("user"===a?-1:1)+")"}_getFacingMode(a){return(a=a.getVideoTracks()[0])?/rear|back|environment/i.test(a.label)?"environment":/front|user|face/i.test(a.label)?"user":null:null}static _drawToCanvas(a,c,d,f,g=!1){d=d||document.createElement("canvas");f=f||document.createElement("canvas");var h=c&&c.x?c.x:0;let p=c&&c.y?c.y:0,n=c&&c.width?c.width:a.videoWidth||a.width,r=c&&c.height?c.height:a.videoHeight||a.height;var k=a.videoWidth||
a.width;let t=a.videoHeight||a.height;g||(g=c&&c.downScaledWidth?c.downScaledWidth:n,c=c&&c.downScaledHeight?c.downScaledHeight:r,d.width!==g&&(d.width=g),d.height!==c&&(d.height=c),f.width!==k&&(f.width=k),f.height!==t&&(f.height=t));k=d.getContext("2d",{alpha:!1});k.imageSmoothingEnabled=!1;k.drawImage(a,h,p,n,r,0,0,d.width,d.height);h=f.getContext("2d",{alpha:!1});h.imageSmoothingEnabled=!1;h.drawImage(a,0,0,f.width,f.height);return[d,k,f,h]}static _loadImage(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(a instanceof
